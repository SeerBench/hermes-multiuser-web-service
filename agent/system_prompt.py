...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    if not agent.skip_context_files:
        # Use TERMINAL_CWD (env or web_chat ContextVar override) for
        # context-file discovery.  The gateway process often runs from the
        # hermes-agent install dir, so os.getcwd() would pick up the repo's
        # AGENTS.md and other dev files — inflating token usage and leaking
        # operator paths into multi-user sessions.
