...
本 fork 在 commit `2751078b8`（*"feat(web_chat)!: replace local auth with new-api key login, drop quota"*）中**主动删除**了本地 quota 模块 —— 使用量计费现在完全由上游 **new-api** 接管，这是有意的设计。`web_skill_install` 因此**不**维护任何本地字节计数器。§3.4 中的单文件 64 KB / 单 skill 256 KB 硬上限是唯一的磁盘占用保护，且已足够：单个租户每个 skill 最多 ~256 KB，用户的整体 workspace 体积已经由 operator 部署策略（容器磁盘、专用 UID 等 —— 见 `docs/user-guide/web-chat.md` § Per-user isolation）兜底。

如果 new-api 后续暴露了磁盘空间维度，把 skill 安装计入它只需要在 install handler 末尾加一行；纯增量，不影响这里的 API 形态。
...
**Q3.** 大小上限 —— 64 KB / 文件、256 KB / skill —— 是否通过 `web_chat.skill_limits.*` 键支持配置？（注意改名：这些是磁盘占用保护，不是计费维度；配额本身由 new-api 接管，见 §3.5。）*推荐：默认硬编码上线。只有在 operator 主动提出真实需求时再加配置键；默认值足以容纳一份 SKILL.md 加若干 reference 文档。*
